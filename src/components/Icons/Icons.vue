          ...
          <div v-for="(group, key) in groupBy(iconsToShow)">
            ...
            <div class="cells">
              <div v-for="icon of group" class="cell">
                  <img class="animated fadeIn" v-on:dragend="dragEnd(icon)" :data-name='icon.image' :src="basePath + 'google-assets/' + icon.image" alt="">
...
export default {
  ...
  methods: {
    ...
    loadData() {
      this.$http.get('./static/icons.json').then((response) => {
    ...
    dragEnd(icon) {
      ...
      const colorExtension = isGlif ? `/${this.selectedColor}` : '';
      const data = {
        name: `${icon.id.replace('ic_', 'ic/').replace('logo_', 'logo/')}${colorExtension}`,
